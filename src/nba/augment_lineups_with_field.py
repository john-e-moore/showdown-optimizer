...
"""
Augment optimized NBA Showdown lineups with quota-balanced field-style lineups.

This script:
  1) Loads an existing optimizer lineups workbook.
  2) Builds additional CPT+UTIL lineups using the shared quota-balanced field
...
The augmented workbook can then be passed directly into the NBA top1% CLI.
"""
...
def _parse_args(argv: List[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Augment an optimizer lineups workbook with quota-balanced "
            "field-style NBA Showdown lineups."
        )
    )
    parser.add_argument(
        "--lineups-excel",
...
def _augment_lineups(
    ...
    lineups_df = pd.read_excel(xls, sheet_name=top1pct_core.LINEUPS_SHEET_NAME)
    ownership_df = pd.read_excel(xls, sheet_name=top1pct_core.OWNERSHIP_SHEET_NAME)
    projections_df = pd.read_excel(xls, sheet_name=top1pct_core.PROJECTIONS_SHEET_NAME)
    ...
    # Annotate field lineups with projection/salary/stack metadata in an NBA-
    # aware schema (CPT + util1–util5).
    ...
    field_with_meta = field_with_meta.rename(columns={"stack_pattern": "target_stack_pattern"})
    ...
    if "target_stack_pattern" in field_with_meta.columns:
