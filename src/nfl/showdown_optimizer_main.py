...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "If omitted, a timestamped file is written under outputs/nfl/lineups/."
        ),
    )
    parser.add_argument(
        "--num-workers",
    ...
    if args.stack_mode == "none":
        ...
        def _optimize_for_stack_pattern(
            ...
            *,
            ...
            elapsed_pattern = time.perf_counter() - start
            ...
            metrics: dict[str, object] = {
                "pattern": pattern,
                "seconds": elapsed_pattern,
                "num_lineups_requested": int(n),
                "num_lineups_generated": int(len(pattern_lineups)),
                "team_a": team_a,
                "team_b": team_b,
                "team_a_count": int(team_a_count),
                "team_b_count": int(team_b_count),
            }
            return pattern_lineups, label, metrics
        ...
        if args.parallel_mode == "by_stack_pattern" and args.num_workers > 1:
            with ThreadPoolExecutor(max_workers=args.num_workers) as executor:
                ...
                for (pattern, _n, _a_count, _b_count), fut in zip(
                    pattern_jobs, futures
                ):
