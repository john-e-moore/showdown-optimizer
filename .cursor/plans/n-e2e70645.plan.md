...
- [ ] Create project directories and add config module with paths and column mappings.
- [ ] Implement Parquet loading and filtering for player-game and games data with canonical columns.
- [ ] Implement DraftKings offensive fantasy point computation and integrate with loaded player-game stats.
- [ ] Implement per-player DK stats, z-score calculation, offensive player filtering, and save processed parquet.
- [ ] Build pairwise training dataset with player-level and game-level features and target y=z_A*z_B.
