...
"""
NBA-specific wrappers around shared DKEntries helpers.

This mirrors `src.nfl.dkentries_utils` but routes resolution through the
NBA data directory.
"""
...
def resolve_latest_dkentries_csv(explicit: Optional[str] = None) -> Path:
    """
    Resolve the path to a DKEntries CSV for NBA.
    """
...
def _parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Helpers for DraftKings DKEntries CSV templates (NBA)."
    )
    parser.add_argument(
        "--dkentries-csv",
        type=str,
        ...
        help=(
            ...
            "If omitted, the latest DKEntries*.csv under data/nba/dkentries/ is used."
        ),
    )
    parser.add_argument(
        "--count-entries",
