...
"""
Entry point for the NBA Showdown simulation-based correlation pipeline.

This script:
  1. Loads Sabersim projections for a single NBA Showdown slate.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="NBA Showdown player correlation pipeline"
    )
    parser.add_argument(
        "--sabersim-csv",
        type=str,
        default=config.SABERSIM_CSV,
        help="Path to Sabersim NBA Showdown projections CSV.",
    )
    parser.add_argument(
        "--output-excel",
        type=str,
        ...
        help=(
            "Path to output Excel file containing projections and correlation "
            "matrix."
        ),
    )
    parser.add_argument(
    ...
    print(f"Loading NBA Sabersim projections from {args.sabersim_csv}...")
    sabersim_df = sabersim_parser.load_sabersim_projections(args.sabersim_csv)

    print(
        f"Building NBA player correlation matrix via simulation "
    ...
    print(f"Writing NBA correlation Excel output to {args.output_excel}...")
